...
import { PRINT_MATRIX, PRINT_MATRIX_SLUGS } from '../swadpia-print-matrix'
...
// OMO-3627: 성원 인쇄단가 매트릭스(OMO-3623 표집) 재분류 + 보드 가격 게이트.
// 10종(전단·포스터·브로셔·책자·캘린더)은 SWADPIA_MATRIX_ROUTING 활성 시에만 표집 base 채택.
// 기본 OFF(dormant)면 현행 quote-only(last-good 보존) 동작을 100% 유지해야 한다.
describe('OMO-3627 — 인쇄단가 매트릭스 재분류 (라우팅 게이트)', () => {
  const PREV = process.env.SWADPIA_MATRIX_ROUTING

  afterEach(() => {
    if (PREV === undefined) delete process.env.SWADPIA_MATRIX_ROUTING
    else process.env.SWADPIA_MATRIX_ROUTING = PREV
  })

  const garbageGrid = (code: string): SwadpiaCategoryData =>
    categoryData({
      categoryCode: code,
      printEntries: [entry(1, 'GRID', 64_000), entry(2, 'GRID', 64_000)],
    })

  it('매트릭스는 10종을 커버한다 (OMO-3623 표집 slug 전부)', () => {
    expect(PRINT_MATRIX_SLUGS.size).toBe(10)
    for (const slug of [
      'leaflets', 'posters', 'brochures', 'menus',
      'saddle-stitch-booklet', 'perfect-bound-booklet', 'catalogs',
      'wall-calendars', 'desk-calendars', 'mini-calendars',
    ]) {
      expect(PRINT_MATRIX_SLUGS.has(slug)).toBe(true)
      expect(PRINT_MATRIX[slug].baseSupplyKrw).toBeGreaterThan(0)
    }
  })

  it('라우팅 OFF(기본): 10종은 quote-only(null) — 현행 동작 보존, garbage 미기록', () => {
    delete process.env.SWADPIA_MATRIX_ROUTING
    for (const slug of PRINT_MATRIX_SLUGS) {
      const r = deriveProductBasePriceKrw(slug, garbageGrid('CPR3000'))
      expect(r.priceKrw).toBeNull()
      expect(r.mode).toBe('quote-only')
    }
  })

  it('라우팅 명시 비활성값(off/0/빈값)도 OFF 로 취급', () => {
    for (const v of ['off', '0', 'false', '']) {
      process.env.SWADPIA_MATRIX_ROUTING = v
      expect(deriveProductBasePriceKrw('leaflets', garbageGrid('CPR3000')).priceKrw).toBeNull()
    }
  })

  it('라우팅 ON: 10종은 표집 base(print-matrix) 를 채택 — garbage 매트릭스 무시', () => {
    process.env.SWADPIA_MATRIX_ROUTING = 'on'
    const r = deriveProductBasePriceKrw('leaflets', garbageGrid('CPR3000'))
    expect(r.mode).toBe('print-matrix')
    expect(r.priceKrw).toBe(PRINT_MATRIX['leaflets'].baseSupplyKrw) // 119800
    expect(r.priceKrw).not.toBe(64_000)
    // 책자(대형 이동)·캘린더도 표집값 그대로
    process.env.SWADPIA_MATRIX_ROUTING = '1'
    expect(deriveProductBasePriceKrw('saddle-stitch-booklet', garbageGrid('CPR4000')).priceKrw).toBe(823_000)
    expect(deriveProductBasePriceKrw('wall-calendars', garbageGrid('CCD1000')).priceKrw).toBe(375_000)
  })

  it('라우팅 ON 이어도 매트릭스 미커버 slug 는 기존 경로 그대로 (스코프 한정)', () => {
    process.env.SWADPIA_MATRIX_ROUTING = 'on'
    // stickers 는 quote-only 유지, business-cards 는 matrix 경로 유지
    expect(deriveProductBasePriceKrw('stickers', categoryData({ categoryCode: 'CST1000' })).mode).toBe('quote-only')
    const bc = deriveProductBasePriceKrw(
      'business-cards',
      categoryData({ categoryCode: 'CNC1000', printEntries: [entry(200, 'PA', 4_000)] }),
    )
    expect(bc.mode).toBe('matrix')
    expect(bc.priceKrw).toBe(4_000)
  })
})
