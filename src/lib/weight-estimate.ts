...
/**
 * 총 무게(kg)에 맞는 배송 박스의 외형 치수(cm)를 반환한다(OMO-3458 부피무게).
 *
 * FedEx 국제운임은 실측 무게(actual)와 부피무게(dimensional weight) 중 큰 값으로
 * 과금되므로, fetchFedexRates 에 박스 치수를 함께 넘겨 부피무게가 운임에 반영되도록 한다.
 * pickBox 와 동일한 BOX_TIERS 를 사용하되, 입력을 총 무게(g) 기준으로 두어
 * 작은 오차는 한 단계 큰 박스(=더 큰 치수)로 보수적으로 잡는다(과소청구 방지).
 */
export function boxDimsCmForWeight(totalKg: number): {
  lengthCm: number
  widthCm: number
  heightCm: number
} {
  const g = Math.max(0, Number(totalKg) || 0) * 1000
  const { tier } = pickBox(g)
  const dims = tier?.innerDims ?? BOX_TIERS[BOX_TIERS.length - 1].innerDims
  return {
    lengthCm: Math.max(1, Math.round(dims.l / 10)),
    widthCm: Math.max(1, Math.round(dims.w / 10)),
    heightCm: Math.max(1, Math.round(dims.h / 10)),
  }
}
