...
if ([RawPrinter]::Send($Printer, $bytes)) { Write-Host "출력 완료 -> $Printer" -ForegroundColor Green }
  ...
  return (
    <main className="mx-auto max-w-3xl px-6 py-12 space-y-6">
      ...
      {/* 가장 쉬운 방법: 다운로드 후 실행만 (Windows USB) */}
      <section className="rounded-2xl border-2 border-emerald-300 bg-emerald-50/50 p-5 space-y-2">
        <h2 className="text-sm font-semibold text-gray-900 flex items-center gap-2">
          <Download className="h-4 w-4" /> ⭐ 가장 쉬운 방법 — 파일 받아서 실행만 (Windows USB)
        </h2>
        <p className="text-xs text-gray-600">
          이 파일 하나만 받아서 실행하면, 새 라벨을 <b>byte-perfect raw</b>로 받아 USB 프린터로 바로 출력합니다. (브리지·node 불필요)
        </p>
        <a href="/fedex-status/print-fedex-test-label.ps1" download
          className="inline-flex items-center gap-1.5 rounded-lg bg-emerald-600 px-4 py-2 text-sm font-medium text-white">
          <Download className="h-4 w-4" /> 출력 스크립트 다운로드 (print-fedex-test-label.ps1)
        </a>
        <p className="text-xs text-gray-700">받은 뒤 PowerShell에서 1줄:</p>
        <code className="block bg-white px-2 py-1 rounded font-mono text-[11px] break-all">powershell -ExecutionPolicy Bypass -File "$HOME\Downloads\print-fedex-test-label.ps1"</code>
        <p className="text-[11px] text-gray-500">프린터가 다르면 파일 맨 위 <code className="bg-white px-1 rounded">$Printer</code> 이름만 변경. 2D에 흰 틈이 남으면 <code className="bg-white px-1 rounded">$Slow=$true</code>.</p>
      </section>

      {/* 2) 자동 출력 설정 — 1회만, 이후 버튼만 누르면 출력 (브리지 방식) */}
      <section className="rounded-2xl border border-indigo-200 bg-indigo-50/40 p-5 space-y-3">
        <h2 className="text-sm font-semibold text-gray-900 flex items-center gap-2">
          <Terminal className="h-4 w-4" /> 2. (대안) 브리지 방식 — 1회 실행 후 웹 버튼으로 출력
        </h2>
        <p className="text-xs text-gray-600">
