...
// OMO-3520 비고: 후가공 수량의존 단가(보드 지시 "수량 따라 금액 증가")는
// OMO-3567 의 FINISHING_MATRIX_ROUTING 플래그(DORMANT) + cardFinishingWholesaleKrw
// 매트릭스 경로로 일원화됐다(라이브 고객가 컷오버는 보드 게이트). 본 파일은 정액/면적
// (플래그 OFF·현행) 경로만 회귀 검증하며, 수량의존 매트릭스 회귀는 OMO-3567 테스트가 소유한다.
