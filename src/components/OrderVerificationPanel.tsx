...
export function OrderVerificationPanel({
  ...
  // OMO-3058: 무료배송(패키지)은 운임을 단가에 흡수하므로 배송마진만 보면 항상 음수.
  // 순손익(상품마진 − 실 배송원가)으로 판단해야 free_shipping 주문이 오탐되지 않는다.
  const marginPositive = productMargin > 0 && (!hasShipment || totalMargin >= 0)
  ...
  return (
    <div className="bg-white rounded-lg shadow p-5 space-y-5 border-l-4 border-indigo-400">
      ...
      {/* OMO-3058: 실측 배송비 반영 순손익 경고 — 패키지 손해 방지 */}
      {hasShipment ? (
        totalMargin < 0 ? (
          <div className="rounded-lg border-2 border-red-400 bg-red-50 p-4">
            <div className="flex items-center gap-2 flex-wrap">
              <span className="text-red-700 font-bold text-sm">⚠ 순손해 주문 — 발송 보류 검토</span>
              <span className="px-2 py-0.5 rounded text-xs font-bold bg-red-600 text-white">{usd(totalMargin)}</span>
            </div>
            <p className="text-xs text-red-700 mt-1.5 leading-relaxed">
              실측 배송비(<b>{usd(actualShipCost)}</b>) 반영 후 이 주문은 <b>{usd(Math.abs(totalMargin))} 손해</b>입니다.
              상품가 조정·고객 추가청구·사양 협의 중 택일하거나, 손해를 감수하고 발송할지 결정하세요.
            </p>
          </div>
        ) : (
          <div className="rounded-lg border border-green-300 bg-green-50 px-4 py-2.5 flex items-center justify-between gap-2 flex-wrap">
            <span className="text-green-800 font-semibold text-sm">✓ 순이익 주문 (실측 배송비 반영)</span>
            <span className="text-green-800 font-bold text-sm">{usd(totalMargin)} 이익</span>
          </div>
        )
      ) : (
        customerShip <= 0 && (
          <div className="rounded-lg border border-amber-300 bg-amber-50 px-4 py-2.5">
            <span className="text-amber-800 text-sm font-medium">⚖ 실측 배송비 미확정 (무료배송/패키지 주문)</span>
            <p className="text-xs text-amber-700 mt-0.5 leading-relaxed">
              <b>송장 생성 시 실측 무게를 입력</b>하면 실 FedEx 배송비가 반영돼 이 주문이 이익인지 손해인지 확정됩니다. 무게 입력 전 발송 금지.
            </p>
          </div>
        )
      )}
