...
// 자기-숨김(hideOwnRunIssue)은 "스크립트가 실행된" 사이클만 숨긴다. 그러나 러너 인증 실패
// (claude_auth_required)로 blocked 된 실행 이슈는 스크립트가 아예 돌지 못해 숨김이 누락된다.
// → 인증 복구 후 첫 정상 사이클이 본 루틴의 최근 실행 이슈 중 아직 보이는(hiddenAt 없는) 것들을
//   모두 숨겨, 리스트가 스스로 치유되게 한다(blocked 잔재 자동 정리, 재발 방지).
// best-effort: 어떤 실패도 ingest 성공을 무효화하지 않는다.
async function sweepVisibleRoutineIssues() {
  if (NO_HIDE) return;
  if (!PC_API || !PC_KEY) return;
  try {
    const runsRes = await fetch(`${PC_API}/api/routines/${ROUTINE_ID}/runs?limit=50`, {
      headers: { Authorization: `Bearer ${PC_KEY}` },
    });
    if (!runsRes.ok) {
